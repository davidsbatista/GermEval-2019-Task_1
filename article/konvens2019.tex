...
This paper is organized as follows, in Section~\ref{task} we describe task in detail and give a description of the
dataset provided. In Section~\ref{hierarchical-clf} we describe some of the proposed approaches in the literature to
performa hierarchical document classification. In Section~\ref{system} we describe the system we build to tackle the task
and the approaches taken. Section~\ref{experiments} details the experiments done and the results, finally in Section~\ref{future} we outline some future work and ideas.
...
We randomly split the development data into training, 70\% and test sets 30\%.

\subsubsection{Local classifier per node}

For the root node we trained a logistic regression with TF-IDF weighted vectors
in a one-versus-rest scenario using the scikit-learn~\cite{scikit-learn~\cite{Pedregosa:2011:SML:1953048.2078195}} implementation on version 0.21.1

\begin{comment}

root node:
- logistic regression with tf-idf weighted vectors
- trained in a OneVsRestClassifier scenario using
- class_weight='balanced', solver='sag', max_iter=50000

- parameters:
  - tokenization? We use the standard tokenization for german from NLTK~\cite{Bird:2009:NLP:1717171} version XX.XX
  - tfidf: n_gram_range=(1,2)
  parameters = {
...
\hline\centering\textbf{Task}  & \textbf{Precision} &  \textbf{Recall} &  \textbf{F\_{1}}\\
\hline
Subtask\_a              & 0.8247 & 0.8291 & 0.8269 \\
Subtask\_b              & 0.6930 & 0.5602 & 0.6196 \\
\hline
\end{tabular}
\end{center}
\caption{\label{devset-results}Results for the development set.}
\end{table}
