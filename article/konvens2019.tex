...
In our developed system we tried out two main approaches, a classifier per parent node (parent-node), and second one based exploring the labels co-occurrence.
...
In the local classifier approach we trained a classifier per parent node, looking at the data in Table~\ref{} one can see the possible labels per level, in the first level we have 8 possible labels, in the
